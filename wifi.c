...
static const char SUPPLICANT_P2P_NAME[]     = "p2p_supplicant";
static const char SUPP_PROP_NAME[]      = "init.svc.wpa_supplicant";
static const char SUPP_P2P_PROP_NAME[]      = "init.svc.p2p_supplicant";
...
int check_wifi_chip_type(void)
{
	...
	if( wififd < 0 ){
		LOGE("Can't open %s, errno = %d", WIFI_CHIP_TYPE_PATH, errno);
		goto done;
	}//else
	...
	if(0 == strncmp(buf, "RTL8188", strlen("RTL8188")) )
	{
		wifi_chip_type = RTL8188CU;
		LOGD("Read wifi chip type OK ! wifi_chip_type = RTL8188CU");
	} 
	else if (0 == strncmp(buf, "BCM4329", strlen("BCM4329")) )
	{
		wifi_chip_type = BCM4329;
		LOGD("Read wifi chip type OK ! wifi_chip_type = BCM4329");
	}
	else if (0 == strncmp(buf, "BCM4330", strlen("BCM4330")) )
	{
		wifi_chip_type = BCM4330;
		LOGD("Read wifi chip type OK ! wifi_chip_type = BCM4330");
	}
	else if (0 == strncmp(buf, "RK901", strlen("RK901")) )
	{
		wifi_chip_type = RK901;
		LOGD("Read wifi chip type OK ! wifi_chip_type = RK901");
	}
	else if (0 == strncmp(buf, "RK903", strlen("RK903")) )
	{
		wifi_chip_type = RK903;
		LOGD("Read wifi chip type OK ! wifi_chip_type = RK903");
	}
...
static int check_wireless_ready(void)
{
	...
	if (timeout <= 0)
	{
       	LOGD("Timeout to wait for Wifi interface within 15 seconds.\n");
...
{
    char driver_status[PROPERTY_VALUE_MAX];
    int count = 150; /* wait at most 15 seconds for completion */
    int type;
	char path[64];
	...
	LOGD("%s: DRIVER_MODULE_PATH = %s, DRIVER_MODULE_ARG = %s", __FUNCTION__, path, DRIVER_MODULE_ARG);
    if (is_wifi_driver_loaded()) {
        return 0;
    }
	...
	{
		/* 
		 * Try to remove module due to driver may initiate fail
		 * but doesn't release the request driver. -- Yongle Lai
		 */
        LOGD("%s: driver load failed", __FUNCTION__);
...
int wifi_start_supplicant_common(const char *config_file)
{
	...
	char supp_prop_name[50];
	char supplicant_name[50];
#ifdef HAVE_LIBC_SYSTEM_PROPERTIES
    const prop_info *pi;
    unsigned serial = 0;
#endif

	memset(supp_prop_name, 0, 50);
	memset(supplicant_name, 0, 50);
	if(strncmp(config_file, P2P_CONFIG_FILE, strlen(config_file)) == 0){
		strncpy(supp_prop_name, SUPP_P2P_PROP_NAME, strlen(SUPP_P2P_PROP_NAME));
		strncpy(supplicant_name, SUPPLICANT_P2P_NAME, strlen(SUPPLICANT_P2P_NAME));
		LOGD("Starting wpa_supplicant_p2p deamon, supp_prop_name = %s, supplicant_name = %s", supp_prop_name, supplicant_name);
	} else {
		strncpy(supp_prop_name, SUPP_PROP_NAME, strlen(SUPP_PROP_NAME));
		strncpy(supplicant_name, SUPPLICANT_NAME, strlen(SUPPLICANT_NAME));
		LOGD("Starting wpa_supplicant deamon, supp_prop_name = %s, supplicant_name = %s", supp_prop_name, supplicant_name);
	}
    ...
    while (count-- > 0) {
        ...
        if (pi != NULL) {
            ...
            if (strcmp(supp_status, "running") == 0) {
                LOGD("%s: supp_status = %s", __FUNCTION__, supp_status);
                return 0;
	...
	LOGD("%s: failed, supp_status = %s", __FUNCTION__, supp_status);
    return -1;
}
...
int wifi_stop_supplicant()
{
    ...
    if (property_get(SUPP_PROP_NAME, supp_status, NULL)
        && strcmp(supp_status, "stopped") == 0) {
        if (property_get(SUPP_P2P_PROP_NAME, supp_status, NULL)
			        && strcmp(supp_status, "stopped") == 0) {
			return 0;
		} else
			deamon = 1; // wpa_supplicant_p2p is running
    } else
		deamon = 0; // wpa_supplicant is running

    if (deamon) 
		property_set("ctl.stop", SUPPLICANT_P2P_NAME);
    else property_set("ctl.stop", SUPPLICANT_NAME);
	sched_yield();
...
conn_retry:
    ...
    LOGD("open connection to supplicant on %s success. count=%d", ifname, count);
    monitor_conn = wpa_ctrl_open(ifname);
    if (monitor_conn == NULL) {
		LOGE("%s: monitor_conn fail.", __FUNCTION__);
        wpa_ctrl_close(ctrl_conn);
        ctrl_conn = NULL;
        return -1;
    }
    if (wpa_ctrl_attach(monitor_conn) != 0) {
		LOGE("%s: wpa_ctrl_attach fail.", __FUNCTION__);
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, exit_sockets) == -1) {
		LOGE("%s: socketpair fail.", __FUNCTION__);
        wpa_ctrl_close(monitor_conn);
        wpa_ctrl_close(ctrl_conn);
        ctrl_conn = monitor_conn = NULL;
        return -1;
...
const char *wifi_get_fw_path(int fw_type)
{
    ...
    case WIFI_GET_FW_PATH_AP:
        if((chip_type == BCM4330) || (chip_type == RK903))
            return "/system/etc/firmware/fw_bcm4330_apsta.bin";
        else
            return WIFI_DRIVER_FW_PATH_AP;
    case WIFI_GET_FW_PATH_P2P:
		if((chip_type == BCM4330) || (chip_type == RK903))
			return "/system/etc/firmware/fw_RK903_p2p.bin";
		else
...
int is_wifi_support_direct()
{
	int wififd;
	char buf[64];
					
	wififd = open("/sys/class/rkwifi/p2p", O_RDONLY);
	if( wififd < 0 ){
		LOGE("Can't open %s", "/sys/class/rkwifi/p2p");
		goto done;
	}
